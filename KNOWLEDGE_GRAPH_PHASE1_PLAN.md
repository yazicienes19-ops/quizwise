...
## 10. Phase 5A — Fünf gezielte Fixes aus dem Usability-Test, umgesetzt 2026-08-02

Kein Feature-Ausbau, keine neue Architektur. Ziel: ausschließlich die fünf konkreten "Muss vor der Beta"-Punkte aus `KNOWLEDGE_GRAPH_USABILITY_SESSION.md` beheben, jeder Punkt einzeln committet und im Dev-Harness verifiziert, bevor der nächste begann. Die Graph Engine (Phase 3) gilt weiterhin als eingefroren — alle fünf Änderungen liegen ausschließlich in `GraphCanvas.tsx` (UI) plus den beiden minimal nötigen Erweiterungen in `services/graph/types.ts` und `useKnowledgeGraph.ts` (s. u.).

1. **Doppelklick-Bug behoben.** `onDoubleClick={e => e.stopPropagation()}` auf dem Node verhindert, dass ein Doppelklick auf einen bestehenden Node zum Hintergrund durchgereicht wird und dort fälschlich einen zweiten Node erzeugt.
2. **Titel direkt inline editierbar.** Kein Dialog — HTML-`<input>`-Overlay außerhalb des SVG (bestehendes Muster aus Phase 3/altem MindmapCanvas, wegen Safaris Hit-Testing-Bug bei `<foreignObject>` in einem transformierten `<g>`), per Doppelklick auf den Node geöffnet. Text wird beim Öffnen automatisch selektiert (`.select()`) — der erste Tastendruck ersetzt ihn direkt, kein manuelles Markieren nötig. Enter committet, Escape verwirft, Klick außerhalb committet über `onBlur`.
3. **Node löschen über die Entf-Taste.** Bewusst `archiveNode` (Soft Delete, undo-fähig), kein Kontextmenü, keine Bestätigung — reagiert nicht, während Titel/Notiz gerade bearbeitet werden oder während gezogen wird (sonst würde Löschen von Zeichen im Textfeld stattdessen den ganzen Node archivieren).
4. **Genau ein Freitext-Notizfeld pro Node.** `notes` (nicht `description`), reiner Klartext, sichtbar sobald ein Node ausgewählt ist — HTML-`<textarea>`-Overlay nach demselben Muster wie der Titel.
5. **Kein stiller Standard-Beziehungstyp mehr.** `pickDefaultRelationTypeId` vollständig entfernt. Loslassen einer gezogenen Kante über einem Zielnode öffnet stattdessen ein minimales `<input>`-Overlay ("Beziehung eingeben…"); exakte, case-insensitive Übereinstimmung mit einem bestehenden Label nutzt dessen Beziehungstyp weiter, jede andere Eingabe legt über `graphMutationService.createRelationType` einen neuen an. Leer lassen/Escape erzeugt bewusst keine Kante.

**Notwendige, musterkonforme Erweiterung (kein neuer Architekturentscheid, transparent vor der Umsetzung angekündigt):** Punkt 5 kann zur Laufzeit einen neuen `GraphRelationType` erzeugen — `GraphEntityChange` (Domain, `services/graph/types.ts`) bekam dafür einen dritten Fall `{ kind: 'relationType'; entity: GraphRelationType }`, `useKnowledgeGraph`s `onEntityChanged` ruft darauf `persistence.commitRelationType` — die Funktion existierte in `GraphPersistenceService` bereits seit Phase 2, wurde bis hierhin aber von nichts aufgerufen.

**Bewusste Lücke, unverändert seit Phase 3:** Beziehungstyp-Mutationen laufen weiterhin nicht über `GraphHistoryService` (Undo/Redo betrifft sie nicht) — das war schon in Phase 3 eine bewusste Scoping-Entscheidung und wurde hier nicht angetastet.

**Verifiziert:** jeder der fünf Punkte einzeln im Dev-Harness per Playwright nachgestellt (Rename, Notiz speichern, Entf-Taste, Kante mit bewusst gewähltem Label, Doppelklick erzeugt keinen Geister-Node mehr) — jeweils ohne Konsolen-/Seitenfehler. `npx vitest run`: 685/685 Tests grün.

### Erneuter Lernsitzungs-Test (Nutzer-Vorgabe: dieselbe Sitzung aus `KNOWLEDGE_GRAPH_USABILITY_SESSION.md` wiederholen)

Ergebnis s. eigener Abschnitt am Ende von `KNOWLEDGE_GRAPH_USABILITY_SESSION.md`. Kurzfassung: alle fünf "Muss vor der Beta"-Blocker sind im echten Test bestätigt behoben, der Lernworkflow (Thema strukturieren, benennen, verstehen-in-Worten-festhalten, Fehler korrigieren) funktioniert jetzt durchgehend. Drei bereits bekannte, bewusst zurückgestellte Lücken aus der "Sollte vor dem Launch"-Liste (Kanten-Beschriftung sichtbar machen, Kante nachträglich bearbeitbar machen) wurden im Test erneut bestätigt und um einen zusätzlichen, bisher nicht dokumentierten Befund ergänzt: Ein Versuch, dieselbe Beziehung doppelt anzulegen, wird zwar korrekt abgelehnt, aber ohne jede sichtbare Rückmeldung — die Eingabe schließt sich kommentarlos. Keiner dieser drei Punkte wurde in dieser Phase behoben (explizit außerhalb des Scopes von Phase 5A).

---

## 11. Phase 5B — Relationship UX, umgesetzt 2026-08-02

Auftrag des Nutzers: ausschließlich die drei aus dem Phase-5A-Nachtest übrig gebliebenen Kanten-Punkte, Engine weiterhin eingefroren, keine Sidebar/Dokumente/KI/Karteikarten/Quiz, keine neuen Ideen.

1. **Duplikat-Feedback.** `commitEdgePrompt` (GraphCanvas.tsx) schließt das "Beziehung eingeben…"-Prompt bei Ablehnung nicht mehr sofort — der ohnehin schon verständliche, undokumentierte Ablehnungsgrund aus `GraphValidationService` (z.B. `validateNoDuplicateEdge`s Meldung) wird jetzt direkt unter der Eingabe angezeigt (`edgePromptError`-State), der Nutzer kann korrigieren oder mit Escape bewusst abbrechen. Dieselbe Rückmeldung gilt jetzt auch beim Umbenennen einer bestehenden Kante (Punkt 3).
2. **Sichtbare Kantenbeschriftung.** Vor der Umsetzung mit dem Nutzer abgestimmt (explizite Vorgabe: "Falls dabei Layout-Probleme entstehen könnten, schlage zuerst eine Lösung vor"): horizontales `<text>` mit Hintergrund-Pille am Kantenmittelpunkt (`computeEdgeGeometry`), bewusst nicht mit der Kante rotiert — bleibt bei jedem Winkel aufrecht lesbar wie Node-Titel. Layout-Risiko benannt und vom Nutzer bestätigt: zwei unterschiedliche Beziehungstypen zwischen demselben Node-Paar sind laut Domain-Modell erlaubt (nur inhaltliche Duplikate sind blockiert) und würden ohne Gegenmaßnahme deckungsgleiche Label erzeugen — gelöst über `edgeParallelIndex` (gruppiert Kanten nach ungeordnetem Node-Paar, versetzt nur die Label-Position pro Kante innerhalb ihrer Gruppe). Reine Anzeigekorrektur, keine Änderung an Linien-Geometrie oder Layout-Engine.
3. **Beziehung ansehen/ändern/löschen.** `GraphSelectionState` um `selectedEdgeId` erweitert (`services/graph/graphSelectionService.ts`), bewusst gegenseitig ausschließend mit `selectedNodeId` — nie zwei kontextabhängige Overlays gleichzeitig. Kanten haben jetzt einen breiten, unsichtbaren Hit-Bereich (14px) neben der 2px sichtbaren Linie und sind damit klickbar wie Nodes. Auswahl zeigt ein Editier-Overlay am Kantenmittelpunkt (vorbelegt mit dem aktuellen Beziehungstyp) — Ändern läuft über dieselbe Freitext-Logik wie das Anlegen (`resolveRelationTypeId`, aus `commitEdgePrompt` herausgezogen und von beiden Flows geteilt). Löschen über einen Button im Overlay oder die Entf-Taste — derselbe Handler wie für Nodes (aus Phase 5A), nur um den Kanten-Fall erweitert; reagiert nicht, während das Kanten-Label gerade editiert wird (`isEditingEdgeLabel`, gleiche Schutzlogik wie bei Titel/Notiz in Phase 5A).

**Kein neuer Architekturentscheid, nur ein neuer Bug bei der Verifikation gefunden+richtig eingeordnet:** Ein Playwright-Testlauf zeigte scheinbar, dass die Entf-Taste eine per Klick ausgewählte Kante nicht löscht. Ursache war das Testskript selbst: es hatte zusätzlich `Tab` gedrückt, um den Fokus "wegzunehmen" — dabei landete der native Tab-Fokus (mangels vorherigem Fokus, da das Editier-Input bewusst NICHT automatisch fokussiert wird, s. Punkt 3) zurück im Editier-Input, wodurch die absichtliche Schutzlogik (`isEditingEdgeLabel`) die Entf-Taste korrekt als Text-Editier-Eingabe statt als Lösch-Befehl behandelte — exakt richtiges Verhalten. Ohne den unnötigen Tab-Zwischenschritt (Klick direkt gefolgt von Entf, wie ein echter Nutzer es täte) funktioniert das Löschen zuverlässig.
