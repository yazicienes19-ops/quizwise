...
5. ~~🟠 **Wissensnetz Last-Write-Wins**~~ → **ERLEDIGT/FEHLALARM (geprüft 2026-09-10)**: Fix war bereits vollständig umgesetzt (nicht dokumentiert) — `useKnowledgeGraph.ts` exponiert `getState()`/`stateRef`, GraphCanvas.tsx liest JEDEN Commit-Aufruf über `stateForCommit()` (Zeile 716, `getState ? getState() : state`), alle 8 Commit-Stellen (Titel-Edit, Node-Drag, Kante anlegen/umbenennen/löschen, Node anlegen/löschen) nutzen es bereits, inkl. explizitem "LWW-Fix"-Kommentar bei der Kanten-Umbenennung (Zeile 1460). Diese Audit-Notiz war schlicht nicht als erledigt markiert.
...
9. 🟠 **Testlücken schließen** (höchster Nutzen zuerst): AnkiImport-Parsing, useDocuments-Upload-Pfade, examHistory/savedExams, errorMessages *(✅ erledigt 2026-08-23)*, recallHistoryService, relationType/nodeDocumentRef-Commits. ~~⚠️ die beiden `waitFor`-basierten stateRef/Commit-Tests in `useKnowledgeGraph.test.ts` sind unter Volllast flaky~~ → **ERLEDIGT (2026-09-10)**: `waitFor`-Timeout für genau diese 2 Tests von Default 1000ms auf 5000ms erhöht (kein echtes Timing-Verhalten wird geprüft, nur mehr Headroom gegen Mikrotask-Verzögerung unter paralleler CPU-Last) — volle Suite (87 Dateien/1052 Tests) danach 2× hintereinander grün.
