...
export const AppContent: React.FC<AppContentProps> = (p) => {
  ...
  if (isLoading && activeTab !== ActiveTab.EXAM && activeTab !== ActiveTab.QUIZ && activeTab !== ActiveTab.PLANNER && activeTab !== ActiveTab.RECALL && activeTab !== ActiveTab.LIBRARY) {
    return (
      <div className="flex flex-col items-center justify-center py-20 lg:py-32 space-y-8 animate-in fade-in zoom-in-95 duration-500 px-4">
        ...
        <p className="text-2xl font-semibold tracking-tighter text-slate-900 dark:text-white">{t('ac.oneMoment')}</p>
      </div>
    );
  }
  ...
  switch (activeTab) {
      ...
      if (isLoading) return (
        <div className="flex flex-col items-center justify-center py-32 space-y-6 animate-in fade-in duration-500 px-4">
          ...
          <div className="text-center space-y-1">
            <p className="text-lg font-semibold tracking-tighter dark:text-white">{t('ac.generatingQuiz')}</p>
    ...
    case ActiveTab.RADAR:
      return <LearningCoach metrics={metrics} decks={decks} onNavigate={setActiveTab} onAction={handleWeakTopicAction} onCreateCardsFromErrors={handleCreateCardsFromErrors} flowResult={flowResult} examTerms={examTerms} activeModule={collections.find(c => c.id === activeModuleId) ?? null} documents={documents} userId={user?.id} collections={collections} onModuleChange={p.onModuleChange} />;
