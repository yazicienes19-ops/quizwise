...
import { sessionsForDate, applySessionSave, SessionFormInput } from '../services/calendarSessions';
import { toast } from '../services/toast';
import { useTranslation } from '../i18n/I18nProvider';
import { formatDate } from '../i18n/dates';
import { localeTag } from '../i18n';
import type { TKey } from '../i18n';
import { ChevronLeft, ChevronRight, X, Plus, Repeat as RepeatIcon } from 'lucide-react';
...
function buildMonthCells(
  ...
  while (cur <= endDate) {
    ...
    const items: CalendarItem[] = [
      ...
      ...daySessions.map(s => ({ id: s.id, title: s.topic, source: 'session' as const, color: s.color, recurring: s.recurring })),
    ];
...
export const StudyPlanner: React.FC<StudyPlannerProps> = ({ metrics, decks, examTerms, onUpdateExams, userId, collections, onUpdateCollection }) => {
  ...
  const handleSkipOccurrence = (ruleId: string) => {
    if (!selectedDate) return;
    const dateStr = toDateStr(selectedDate);
  ...
  const handleSmartPlan = async () => {
    ...
    try {
      ...
      toast.success(t('sp2.planCreated', { entries: plan.length, days: tp('sp2.daysN', days.length) }));
        ...
        <div className="max-w-5xl mx-auto rounded-[32px] shadow-3d-raised overflow-hidden" style={{ background: 'var(--bg-sidebar)', border: '1px solid var(--border-color)' }}>
          ...
          <div className="flex items-center gap-4 px-6 py-4 flex-wrap" style={{ borderTop: '1px solid var(--border-color)' }}>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">{t('sp2.addSession')}</span>
            </div>
            {spacedEnabled && (
              <div className="flex items-center gap-1.5">
      ...
      {viewMode === 'monat' && selectedDate && (
        <div className="max-w-5xl mx-auto">
          <CalendarDayPanel
            ...
            onSkipRecurringOccurrence={handleSkipOccurrence}
            ...
            upcomingItems.map(item => {
              ...
              return (
                <div key={item.id} className={`flex items-center gap-4 px-6 py-5 bg-white dark:bg-slate-900 rounded-[20px] border shadow-sm ${item.source === 'exam' ? 'border-rose-100 dark:border-rose-900/30' : 'border-slate-100 dark:border-slate-800'}`}>
                  <div
                    className={`w-2 h-10 rounded-full shrink-0 ${item.source === 'exam' ? 'bg-rose-400' : item.isAuto ? '' : 'bg-blue-400'}`}
                    style={item.isAuto ? { background: 'var(--primary)' } : undefined}
                  />
