...
export const StudyPlanner: React.FC<StudyPlannerProps> = ({ metrics, decks, examTerms, onUpdateExams, userId, collections, onUpdateCollection }) => {
  ...
  const regenerateSpacedPlan = (evs: StudyEvent[]): StudyEvent[] => {
  ...
  useEffect(() => {
    ...
    try { const s = localStorage.getItem('quizwise_recurring_sessions'); if (s) setRecurringSessions(JSON.parse(s)); } catch {}
    try { const s = localStorage.getItem('quizwise_calendar_sessions'); if (s) setCalendarSessions(JSON.parse(s)); } catch {}
    let evs: StudyEvent[] = [];
