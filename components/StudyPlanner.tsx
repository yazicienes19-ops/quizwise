...
import { StudyEntry, StudyEvent, TopicMetric, FlashcardDeck, ExamTerm } from '../types';
...
export const StudyPlanner: React.FC<StudyPlannerProps> = ({ metrics, decks, examTerms, onUpdateExams, userId }) => {
  ...
  const handleToggleSpaced = () => {
    if (!spacedEnabled) {
      ...
      toast.success(autoCount > 0
        ? `${autoCount} Wiederholungs-Session${autoCount !== 1 ? 's' : ''} automatisch geplant`
        : 'Aktiviert. Sobald du lernst oder Klausuren einträgst, werden Wiederholungen geplant.');
    } else {
      ...
      toast.info(removed > 0 ? `Deaktiviert, ${removed} automatische${removed !== 1 ? '' : 'r'} Eintrag${removed !== 1 ? 'e' : ''} entfernt` : 'Automatische Planung deaktiviert');
    }
  };
  ...
  const handleRedistribute = () => {
    ...
    saveEvents(next);
    saveSpacedSettings({ enabled: true, lastRunDay: todayStr }, userId);
    toast.success(`Neu verteilt: ${next.filter(e => e.isAutoGenerated).length} Wiederholungs-Sessions`);
  };
          ...
          >
            ...
            <div>
              ...
              <p className="text-[9px] font-medium text-slate-400">
                {spacedEnabled ? 'Sessions werden zeitversetzt geplant (+1/+3/+7 Tage), nur Auto-Einträge werden verschoben.' : 'Aus, du planst alles selbst.'}
              </p>
            </div>
          </button>
          {spacedEnabled && (
                  ...
                  <div className="space-y-1">
                    {visible.map(item => (
                      <div
                        ...
                        style={item.isAuto ? { background: 'color-mix(in srgb, var(--primary) 12%, transparent)', color: 'var(--primary)' } : undefined}
                      >
                        {item.isAuto && <span className="text-[7px] font-black uppercase tracking-widest mr-1 opacity-70">Auto</span>}
                        {item.title}
                      </div>
                    ))}
                    {overflow > 0 && (
          ...
          <div className="flex items-center gap-4 px-6 py-4" style={{ borderTop: '1px solid var(--border-color)' }}>
            ...
            {spacedEnabled && (
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Auto-Wiederholung</span>
              </div>
            )}
            <div className="flex items-center gap-1.5">
