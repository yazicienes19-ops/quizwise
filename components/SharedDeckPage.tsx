    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--bg-main)' }}>
        <div className="text-center space-y-4">
          ...
          <p className="text-[11px] font-black uppercase tracking-widest text-slate-400">Stapel wird geladen…</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'var(--bg-main)' }}>
        <div className="text-center space-y-4 max-w-sm">
          ...
          <h2 className="text-2xl font-black dark:text-white">Stapel nicht gefunden</h2>
          <p className="text-slate-400 text-sm">Dieser Link ist abgelaufen oder ungültig.</p>
          <button
    ...
    <div className="min-h-screen py-12 px-4" style={{ background: 'var(--bg-main)' }}>
      <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in duration-700">
        ...
        <div className="text-center space-y-3">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Geteilter Stapel</p>
          <h1 className="text-4xl font-black tracking-tighter dark:text-white">{deck.name}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {deck.cards.length} Karte{deck.cards.length !== 1 ? 'n' : ''} · Erstellt {new Date(deck.created_at).toLocaleDateString('de-DE')}
          </p>
        </div>

        {/* Preview cards */}
        <div className="space-y-3">
          <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Vorschau</p>
          {deck.cards.slice(0, 6).map((c, i) => (
            <div key={i} className="grid grid-cols-2 gap-4 px-5 py-4 rounded-[20px]" style={{ background: 'var(--bg-sidebar)', border: '1px solid var(--border-color)' }}>
              <div>
                <p className="text-[8px] font-black uppercase tracking-widest text-slate-400 mb-1">Vorderseite</p>
                <p className="text-sm font-medium dark:text-white">{c.front}</p>
              </div>
              <div>
                <p className="text-[8px] font-black uppercase tracking-widest text-slate-400 mb-1">Rückseite</p>
                <p className="text-sm text-slate-500 dark:text-slate-400">{c.back}</p>
              </div>
            </div>
          ))}
          {deck.cards.length > 6 && (
            <p className="text-[9px] text-slate-400 text-center font-black">+{deck.cards.length - 6} weitere Karten</p>
          )}
        </div>
        ...
        {accepted ? (
          <div className="text-center py-6 space-y-3">
            <p className="text-emerald-600 font-black text-lg">✓ Stapel übernommen</p>
            <button
              onClick={() => window.location.href = '/'}
              className="px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all hover:scale-105"
              style={{ background: 'var(--primary)', color: 'var(--primary-text)' }}
          ...
          >
            ...
            In meine Karten übernehmen
          </button>
        )}

        {!userId && (
          <p className="text-center text-[10px] text-slate-400">
            Du musst eingeloggt sein, um den Stapel zu übernehmen.
          </p>
        )}
      </div>
    </div>
