      ...
      return (
        <div className="max-w-3xl mx-auto py-20 px-4 text-center space-y-4">
          ...
          <div className="flex gap-3 justify-center">
            {onRetryAnalysis && (
              <button onClick={onRetryAnalysis} className="px-6 py-3 rounded-2xl text-[13px] font-semibold" style={{ background: 'var(--primary)', color: 'var(--primary-text)' }}>
                {t('rd.retryAnalysis')}
              </button>
            )}
            <button onClick={onBack} className="px-6 py-3 rounded-2xl text-[13px] font-semibold border" style={{ borderColor: 'var(--border-color)', color: 'var(--text-main)' }}>
              {t('rd.backToLibrary')}
            </button>
          </div>
        </div>
    ...
    <div className="w-full space-y-3 animate-in fade-in duration-700">
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
        ...
        <div className="hidden sm:flex items-center gap-2 shrink-0">
          ...
          <p className="text-[11px] font-medium text-slate-400 whitespace-nowrap">{doneIndices.length} / {chapters.length}</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-10 lg:items-start gap-4">
        ...
        <div className="relative lg:col-span-7 rounded-[24px] flex flex-col max-h-[calc(100vh-21rem)] min-h-[300px] lg:max-h-[calc(100vh-6rem)] lg:min-h-[420px] overflow-hidden" style={{ background: 'var(--bg-main)', border: '1px solid var(--border-color)' }}>
              ...
              <div className="flex-1 min-h-0 overflow-y-auto px-4 py-6 lg:px-8 lg:py-8" onMouseUp={handleTextSelection} onTouchEnd={handleTextSelectionTouch}>
                <article className="max-w-[660px] mx-auto rounded-2xl p-8 lg:p-12" style={{ background: 'var(--bg-sidebar)', border: '1px solid var(--border-color)', boxShadow: '0 10px 30px -5px rgba(0, 0, 0, 0.08), 0 4px 10px -2px rgba(0, 0, 0, 0.03), inset 0 1px 0 0 rgba(255, 255, 255, 0.5)' }}>
                  <div className="flex items-center justify-between gap-3 mb-6">
                    ...
                    {activeDone && (
                      <span className="shrink-0 px-3 py-1 rounded-full text-[11px] font-semibold uppercase tracking-[0.08em] bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400">
                        {t('rd.read')}
                      </span>
                    )}
                  </div>
          ...
          {usesDigest && (
            <p className="shrink-0 px-4 pb-2 lg:px-8 text-[11px] font-medium text-slate-400 text-center">{t('rd.digestSourceHint')}</p>
          )}
        </div>
