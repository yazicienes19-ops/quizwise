    ...
    if (q.type === 'mc' && q.options) {
      const scenarioBlock = q.scenarioText ? (
        <div className="pl-4 lg:pl-10 mb-4 p-5 bg-amber-50 dark:bg-amber-900/20 rounded-[20px] border border-amber-200 dark:border-amber-800">
          <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-amber-600 dark:text-amber-400 mb-2">{t('quiz.badge.scenario')}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pl-4 lg:pl-10">
          {q.options.map((opt, oi) => {
            ...
            return (
              ...
              >
                <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center shrink-0 text-[11px] font-semibold ${
      ...
      if (mode === 'solve') {
        return (
          <div className="pl-4 lg:pl-10 space-y-2">
            <p className="text-xs font-semibold text-slate-400 mb-3">{t('quiz.rankingHint')}</p>
      ...
      return (
        <div className="pl-4 lg:pl-10 space-y-2">
          {correct.map((item, i) => {
            ...
            return (
              <div key={item} className="flex items-center gap-3">
                <span className="w-7 h-7 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 text-[11px] font-semibold flex items-center justify-center shrink-0">{i + 1}</span>
                <div className={`flex-1 p-3 rounded-2xl border-2 text-sm font-medium ${ok ? 'border-emerald-400 bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-300' : 'border-rose-300 bg-rose-50 dark:bg-rose-950/20 text-rose-700 dark:text-rose-300'}`}>
                  {user[i] || '—'}{!ok && <span className="text-emerald-600 dark:text-emerald-400 text-[11px] font-semibold ml-2">→ {item}</span>}
                </div>
              </div>
            );
          })}
      ...
      if (mode === 'solve') {
        ...
        return (
          <div className="pl-4 lg:pl-10">
            ...
            />
            <p className="text-xs font-semibold text-slate-400 mt-2">{t('ev.stepByStepHint')}</p>
          </div>
        );
      }
      if (mode === 'result') {
        ...
        return (
          <div className="pl-4 lg:pl-10 space-y-4">
            {(q.correctApproach !== undefined || q.finalResultCorrect !== undefined) && (
              <div className="flex flex-wrap gap-2">
                {q.correctApproach !== undefined && (
                  <span className={`text-[11px] font-semibold uppercase tracking-[0.08em] px-2.5 py-1 rounded-full ${q.correctApproach ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600' : 'bg-rose-100 dark:bg-rose-900/30 text-rose-600'}`}>
                    {q.correctApproach ? t('ev.stepApproachOk') : t('ev.stepApproachIssue')}
                  </span>
                )}
                {q.finalResultCorrect !== undefined && (
                  <span className={`text-[11px] font-semibold uppercase tracking-[0.08em] px-2.5 py-1 rounded-full ${q.finalResultCorrect ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600' : 'bg-rose-100 dark:bg-rose-900/30 text-rose-600'}`}>
                  ...
                  return (
                    <li key={i} className={`p-3 rounded-2xl border-2 font-mono text-sm ${cls}`}>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {fb && <span className="text-xs font-semibold shrink-0">{verdictLabel[fb.verdict]}</span>}
                      </div>
                      {fb?.note && <p className="text-xs mt-1 font-sans opacity-90">{fb.note}</p>}
                      {fb?.errorType && <p className="text-[11px] mt-1 font-sans font-semibold uppercase tracking-[0.08em] opacity-75">{t('ev.distractorErrorTypeLabel')}: {getDistractorErrorTypeLabel(fb.errorType)}</p>}
                    </li>
                  );
                })}
              </ol>
              ...
              <ul className="space-y-2">
                {missingSteps.map((fb, i) => (
                  <li key={`missing-${i}`} className="p-3 rounded-2xl border-2 border-dashed border-amber-400 bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-300 text-sm">
                    <span className="text-[11px] font-semibold uppercase tracking-[0.08em]">{t('ev.stepVerdictMissing')}</span>
            ...
            {q.expectedSteps && q.expectedSteps.length > 0 && (
              <div className="pt-2">
                <p className="text-xs font-semibold text-slate-400 mb-2">{t('ev.stepExpectedTitle')}</p>
        ...
        <div className="pl-4 lg:pl-10 space-y-4">
          ...
          {q.tfReasonOptions && q.tfReasonOptions.length > 0 && (tfAns?.tf !== undefined || mode === 'result') && (
            <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400">{t('ev.reasoning')}</p>
              {q.tfReasonOptions.map((reason, ri) => {
                ...
                return (
                  ...
                  >
                    {reason}
                    {correct && <span className="ml-2 text-[11px] font-semibold uppercase text-emerald-500">✓</span>}
                  </button>
                );
              })}
            </div>
                ...
                {mode === 'solve' ? (
                  ...
                  <div className={`flex-1 p-3 rounded-2xl text-sm font-medium border ${
                    ...
                    {isWrong && q.matchCorrect && (
                      <span className="block text-[11px] text-emerald-600 dark:text-emerald-400 mt-0.5 font-semibold">
                ...
                <React.Fragment key={pi}>
                  ...
                  {pi < parts.length - 1 && (
                    <span className={`mx-1 inline-flex items-center gap-1 px-2 py-0.5 rounded font-bold ${colorClass}`}>
                      {userBlank || '—'}
                      {match === 'tolerant' && (
                        <span className="text-xs font-semibold opacity-80">{t('ev.blankTolerant')}</span>
                      )}
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-700 pb-32">
      ...
      <div className="flex justify-between items-end border-b-4 border-slate-900 dark:border-slate-100 pb-8">
        <div>
          <h2 className="text-2xl sm:text-4xl font-semibold tracking-tighter dark:text-white">{t('ev.examProtocol')}</h2>
        </div>
      ...
      {mode === 'result' && (
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 ${gradeInfo.bg} dark:bg-slate-900/40 p-5 sm:p-10 rounded-[28px] sm:rounded-[28px] border-2 ${gradeInfo.passed ? 'border-emerald-500' : 'border-rose-500'} animate-in zoom-in-95`}>
          <div className="flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-800 pb-6 md:pb-0">
            ...
            <span className={`text-xs font-semibold mt-2 ${gradeInfo.color}`}>{gradeInfo.label}</span>
          </div>
          <div className="md:col-span-2 space-y-4 flex flex-col justify-center">
              ...
              <div className="pt-2 border-t border-slate-200 dark:border-slate-700">
                {examSaved ? (
                  ...
                  <button onClick={() => setShowSaveInput(true)}
                    className="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-[14px] text-[13px] font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all mb-2">
            ...
            <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-200 dark:border-slate-700">
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-[16px] border-2 border-slate-200 dark:border-slate-700 font-semibold text-[13px] text-slate-600 dark:text-slate-300 hover:border-rose-400 hover:text-rose-600 transition-all"
              >
              ...
              {onNavigate && (
                <>
                  <button onClick={() => onNavigate(ActiveTab.RADAR)} className="px-5 py-2.5 rounded-[16px] border-2 border-slate-200 dark:border-slate-700 font-semibold text-[13px] text-slate-600 dark:text-slate-300 hover:border-indigo-400 hover:text-indigo-600 transition-all">
                    {t('ev.learnAnalysisBtn')}
                  </button>
                  <button onClick={() => onNavigate(ActiveTab.LIBRARY)} className="px-5 py-2.5 rounded-[16px] border-2 border-slate-200 dark:border-slate-700 font-semibold text-[13px] text-slate-600 dark:text-slate-300 hover:border-indigo-400 hover:text-indigo-600 transition-all">
                    {t('ev.toLibrary')}
                  </button>
                </>
              )}
              {onAction && weakestTopic && weakestTopic.score < 70 && (
                <button
                  ...
                  className="px-5 py-2.5 rounded-[16px] font-semibold text-[13px] text-white hover:scale-[1.02] transition-all shadow-lg"
      ...
      {mode === 'result' && scoringProfile && (
        <div className="flex items-center gap-2 flex-wrap px-1">
          <span className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400">{t('ev.scoringProfileLabel')}</span>
          <span className={`text-[11px] font-semibold uppercase tracking-[0.08em] px-2.5 py-1 rounded-full ${scoringProfile.mode === 'strict' ? 'bg-rose-100 dark:bg-rose-950/20 text-rose-600' : scoringProfile.mode === 'lenient' ? 'bg-emerald-100 dark:bg-emerald-950/20 text-emerald-600' : 'bg-indigo-100 dark:bg-indigo-950/20 text-indigo-600'}`}>
          ...
          {scoringProfile.emphases.map(e => (
            <span key={e} className="text-[11px] font-semibold uppercase tracking-[0.08em] px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500">
        ...
        <div className="rounded-[24px] sm:rounded-[24px] p-5 sm:p-8 space-y-6 animate-in fade-in duration-700" style={{ background: 'var(--bg-sidebar)', border: '1px solid var(--border-color)' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {analysis.strengths.length > 0 && (
              <div className="space-y-3">
                <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-emerald-600">{t('ev.strengths')}</p>
                <ul className="space-y-2">
            ...
            {analysis.weaknesses.length > 0 && (
              <div className="space-y-3">
                <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-rose-600">{t('ev.gaps')}</p>
                <ul className="space-y-2">
          ...
          {analysis.recommendations.length > 0 && (
            <div className="pt-4 border-t" style={{ borderColor: 'var(--border-color)' }}>
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-indigo-600 mb-3">{t('ev.recommendations')}</p>
              <div className="flex flex-wrap gap-2">
          ...
          {analysis.topicPerformance.length > 0 && (
            <div className="pt-4 border-t space-y-3" style={{ borderColor: 'var(--border-color)' }}>
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400">{t('ev.topicPerformance')}</p>
      ...
      {mode === 'result' && !analysis && (
        <div className="flex items-center gap-3 px-1 text-slate-400">
          ...
          <span className="text-[11px] font-semibold uppercase tracking-[0.08em]">{t('ev.analysisCreating')}</span>
        </div>
      )}
      ...
      {(mode === 'edit' || mode === 'result') && adaptiveTarget && (
        <div className="rounded-[24px] sm:rounded-[24px] p-5 sm:p-8 space-y-5 animate-in fade-in duration-500" style={{ background: 'var(--bg-sidebar)', border: '1px solid var(--border-color)' }}>
          ...
          {adaptiveTopicCoverage.length > 0 && (
            <div className="space-y-3">
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400">{t('ev.adaptiveTopics')}</p>
          ...
          <div className="space-y-3">
            <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400">{t('ev.adaptiveMix')}</p>
              ...
              {isEditing && tempQuestion ? (
                <div className="space-y-6 animate-in fade-in zoom-in-95 p-5 sm:p-8 bg-[var(--card)] dark:bg-slate-800 rounded-[24px] sm:rounded-[24px] shadow-2xl ring-4 ring-indigo-500/20">
                  ...
                  <div className="flex justify-end gap-3">
                    <button onClick={() => setEditingId(null)} className="text-slate-400 font-semibold text-[13px]">{t('quiz.cancel')}</button>
                    <button onClick={saveEdit} className="bg-indigo-600 text-white px-6 py-2 rounded-xl font-semibold text-[13px]">{t('quiz.save')}</button>
                  </div>
                </div>
              ) : (
                <div className="space-y-6">
                  <div className={`flex justify-between items-start gap-4 ${mode === 'edit' ? 'pr-12' : ''}`}>
                    ...
                    <span className="text-[11px] font-mono bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-lg dark:text-slate-400 uppercase tracking-[0.08em] shrink-0">
                    ...
                    <div className="mt-8 pl-4 lg:pl-10 animate-in slide-in-from-bottom-4 space-y-3">
                      ...
                      <div className={`p-6 rounded-[24px] border-l-4 ${(q.achievedPoints ?? 0) === q.points ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-500' : (q.achievedPoints ?? 0) > 0 ? 'bg-amber-50 dark:bg-amber-950/20 border-amber-500' : 'bg-rose-50 dark:bg-rose-950/20 border-rose-400'}`}>
                        <div className="flex justify-between items-center mb-4">
                          <div className="flex items-center gap-2">
                            ...
                            {q.evaluationConfidence !== undefined && (
                              <span className={`text-[11px] font-semibold uppercase tracking-[0.08em] px-2 py-0.5 rounded-full ${q.evaluationConfidence >= 80 ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600' : q.evaluationConfidence >= 60 ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600' : 'bg-rose-100 dark:bg-rose-900/30 text-rose-600'}`}>
                        ...
                        {q.criterionScores && q.criterionScores.length > 0 && (
                          <div className="space-y-2 mb-4">
                            <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400">{t('ev.rubricTitle')}</p>
                            {q.criterionScores.map((cs, csIdx) => {
                              ...
                              <div key={cs.criterionId} className="flex items-start gap-3 text-sm">
                                <span className={`shrink-0 mt-0.5 w-5 h-5 rounded-lg flex items-center justify-center text-[11px] font-semibold ${cs.status === 'full' ? 'bg-emerald-500 text-white' : cs.status === 'partial' ? 'bg-amber-400 text-white' : 'bg-slate-300 dark:bg-slate-600 text-white'}`}>
                                  {cs.status === 'full' ? '✓' : cs.status === 'partial' ? '~' : '✗'}
                                </span>
                                <div className="flex-1 min-w-0">
                                  <div className="flex justify-between items-center gap-2">
                                    <span className="font-semibold text-[11px] dark:text-white">{cs.criterionName}</span>
                                    <span className={`text-[11px] font-semibold shrink-0 ${cs.status === 'full' ? 'text-emerald-600' : cs.status === 'partial' ? 'text-amber-600' : 'text-slate-400'}`}>
                        ...
                        {q.type === 'mc' && q.selectedDistractorErrorType && (
                          <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                            <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-amber-600 dark:text-amber-400">
                            ...
                            {onStartOperationPractice && (
                              <button
                                ...
                                className="px-3 py-1.5 rounded-xl text-[13px] font-semibold transition-all hover:scale-105 disabled:opacity-60 disabled:hover:scale-100"
                        ...
                        <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
                          <p className="text-[11px] font-semibold uppercase text-slate-400 mb-2">{t('ev.masterSolution')}</p>
                      ...
                      {!questionFeedback[q.id] ? (
                        <div className="flex flex-wrap items-center gap-2 pt-1">
                          <span className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-300 dark:text-slate-600">{t('ev.ratingFair')}</span>
                          {([
                            ...
                            <button
                              ...
                              className={`px-3 py-1.5 rounded-xl text-[13px] font-semibold border-2 border-slate-200 dark:border-slate-700 text-slate-400 transition-all ${fb.cls}`}
                            >{fb.label}</button>
                          ))}
                        </div>
                      ) : (
                        <p className="text-xs font-semibold text-slate-300 dark:text-slate-600 pl-1">
                          {t('ev.fbSaved')}
                        </p>
                      )}
                    </div>
      ...
      {mode === 'solve' && (
        <div className="fixed bottom-[calc(4.5rem+env(safe-area-inset-bottom))] md:bottom-10 left-1/2 -translate-x-1/2 z-[50] flex flex-col items-center gap-3">
            ...
            <div className="bg-[var(--card)] dark:bg-slate-900 rounded-[20px] border border-indigo-200 dark:border-indigo-800 shadow-xl p-4 w-80 animate-in slide-in-from-bottom-4 duration-300">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-3 py-2 bg-indigo-600 text-white rounded-[12px] text-[13px] font-semibold hover:scale-105 transition-all shrink-0"
                >
                  OK
                </button>
          ...
          <div className="flex items-center gap-3">
            {onSaveProgress && (
              <button
                ...
                className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 px-5 py-4 rounded-[24px] font-semibold text-[13px] shadow-lg hover:scale-105 transition-all flex items-center gap-2"
              >
            ...
            <button onClick={handleSubmit} disabled={isEvaluating}
              className="bg-indigo-600 text-white px-6 sm:px-10 py-5 sm:py-6 rounded-[24px] font-semibold text-[13px] shadow-3d-deep hover:scale-105 transition-all flex items-center gap-3 sm:gap-4"
            >
          ...
          {timeLeft !== null ? (
            <p className={`px-6 py-2 rounded-full text-xs font-semibold shadow-lg backdrop-blur-sm transition-all ${
