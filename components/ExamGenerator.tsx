...
export const ExamGenerator: React.FC<ExamGeneratorProps> = ({ onGenerate, isLoading }) => {
  ...
  const processFile = async (file: File): Promise<GenerationSource> => {
  ...
  const handleStart = async () => {
    ...
    try {
      const contentSource = await processFile(contentFile);
      const styleSource = styleFile ? await processFile(styleFile) : undefined;
      onGenerate(contentSource, styleSource, { count: questionCount, difficulty });
    } catch (e) {
      console.error(e);
      // toast is not available here; surface via thrown error to ExamSystem
      throw e;
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-10 lg:space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-700 py-10 px-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
        ...
        <div className="lg:col-span-7 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className={`p-8 bg-white dark:bg-slate-900 rounded-[32px] border-2 transition-all flex flex-col gap-6 shadow-3d-raised ${contentFile ? 'border-indigo-500 ring-4 ring-indigo-500/10' : 'border-slate-100 dark:border-slate-800'}`}>
            <div className="w-16 h-16 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-2xl flex items-center justify-center overflow-hidden">
              <GeneratedImage prompt="Academic books, minimalist illustration" className="w-full h-full object-cover" />
            </div>
            <div className="space-y-1">
              <h3 className="text-xl font-black dark:text-white uppercase tracking-tight">Lernmaterial</h3>
            ...
            <label 
              ...
              className="w-full py-4 bg-slate-900 dark:bg-white dark:text-slate-900 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all text-center cursor-pointer shadow-lg"
            >
              {contentFile ? 'Datei ändern' : 'Datei wählen'}
            </label>
            {contentFile && (
              <div className="p-3 bg-emerald-50 dark:bg-emerald-950/20 rounded-xl flex items-center gap-3">
                 ...
                 <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-black truncate">{contentFile.name}</p>
              </div>
            )}
          </div>

          <div className={`p-8 bg-white dark:bg-slate-900 rounded-[32px] border-2 transition-all flex flex-col gap-6 shadow-3d-raised ${styleFile ? 'border-rose-500 ring-4 ring-rose-500/10' : 'border-dashed border-slate-200 dark:border-slate-700'}`}>
            <div className="w-16 h-16 bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 rounded-2xl flex items-center justify-center overflow-hidden">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-3d-deep p-8 space-y-10">
            ...
            <button
              onClick={handleStart}
              disabled={!contentFile || isLoading}
              className="w-full bg-slate-900 dark:bg-white dark:text-slate-900 text-white py-6 rounded-2xl font-black uppercase tracking-[0.3em] text-[12px] shadow-3d-deep hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-30 disabled:cursor-not-allowed mt-4"
            >
              {isLoading ? (
